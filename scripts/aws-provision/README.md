...
This folder contains IAM policies, user-data, and helper JSON used to run the API on **EC2** with **Docker Compose** and optional **Nova Act** (`Dockerfile.nova-act`).
...
1. **Secrets Manager** → `overemployed/ec2-env` — use **`npm run sync:ec2-secrets`** from a machine that has your `.env`, or edit keys manually in the console (**`GROQ_API_KEY`**, **`NOVA_ACT_API_KEY`**, **`FRONTEND_URL`** / **`API_KEY`**).
